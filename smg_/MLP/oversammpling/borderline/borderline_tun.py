...
adasyn = BorderlineSMOTE(random_state=42)
X_train_resampled, y_train_resampled_indices = adasyn.fit_resample(X_train, y_train_indices)
y_train_resampled = tf.keras.utils.to_categorical(y_train_resampled_indices, num_classes=y_encoded.shape[1])
...
def build_model(hp):
    ...
    model.add(tf.keras.layers.Dense(units=hp.Int('units1', 64, 512, step=64), activation='relu'))
    model.add(tf.keras.layers.BatchNormalization())
    model.add(tf.keras.layers.Dropout(rate=hp.Float('dropout1', 0.1, 0.5, step=0.1)))
    model.add(tf.keras.layers.Dense(units=hp.Int('units2', 32, 256, step=32), activation='relu'))
...
# Custom tuner to pass batch size
...
tuner = MyTuner(
    ...
    project_name='epica_adasyn_tuning'
)
