...
# Train-test split (keeping X_val, y_val separate for unbiased validation)
...
# ADASYN expects 1D labels, so convert y_train from one-hot to integer labels
...
# Ensure all original class columns are present in the resampled one-hot labels
y_train_resampled = pd.get_dummies(y_train_resampled_labels)
# Reindex to ensure all original columns are present, filling missing with 0
y_train_resampled = y_train_resampled.reindex(columns=y_encoded.columns, fill_value=0)


print(f"Original training data shape: X_train={X_train.shape}, y_train={y_train.shape}")
print(f"Resampled training data shape: X_train_resampled={X_train_resampled.shape}, y_train_resampled={y_train_resampled.shape}")


input_dim = X_train_resampled.shape[1] # Number of features (should be same as original X.shape[1])
output_dim = y_train_resampled.shape[1] # Number of classes (should be same as original y_encoded.shape[1])
...
def build_model(hp):
    ...
    """
    # Batch size is now a hyperparameter to be chosen by the tuner
...
class MyTuner(kt.Hyperband):
    def run_trial(self, trial, *args, **kwargs):
        ...
        kwargs['batch_size'] = hp.get('batch_size') # Get batch_size from hyperparameters
...
tuner = MyTuner(
    ...
    max_epochs=10,
    factor=3,
    directory='kt_results_rnn_adasyn', # New directory for RNN + ADASYN results
    project_name='epica_rnn_adasyn_tuning' # New project name
)

# Early stopping callback
stop_early = tf.keras.callbacks.EarlyStopping(monitor='val_loss', patience=3)

# Run search using the ADASYN resampled training data
print("Starting Keras Tuner search for RNN model with ADASYN...")
tuner.search(X_train_resampled, y_train_resampled, validation_data=(X_val, y_val), epochs=10, callbacks=[stop_early], verbose=1)

# Extract best model and hyperparameters
best_hps = tuner.get_best_hyperparameters(1)[0]
print("\n✅ Best Hyperparameters for RNN Model with ADASYN:")
...
# Final training of the best RNN model using ADASYN resampled data
print("\nStarting final training of the best RNN model with ADASYN...")
history = best_model.fit(
    X_train_resampled, y_train_resampled, # Use resampled data here
...
print("\n=== Final Classification Report for RNN Model with ADASYN ===")
