...
# Enhanced with RandomUnderSampler for handling class imbalance
...
class BaselineConfig:
    ...
    # RandomUnderSampler parameters
    use_rus: bool = True  # Set to False to disable RandomUnderSampler
    rus_sampling_strategy: str = "auto"  # 'auto', 'majority', 'not minority', 'all', or dict

    # Class weighting options
    use_class_weight: bool = True  # RandomUnderSampler is undersampling, so class weights may still help
...
class BaselineIDS:
    ...
    def train_and_evaluate(self, X: pd.DataFrame, y: pd.Series):
        ...
        # Apply RandomUnderSampler if enabled
        if self.cfg.use_rus:
            print(f"\nApplying RandomUnderSampler...")
            print(f"  Sampling strategy: {self.cfg.rus_sampling_strategy}")
            ...
            rus = RandomUnderSampler(
                sampling_strategy=self.cfg.rus_sampling_strategy,
                random_state=self.cfg.random_state
            )

            try:
                X_train_scaled, y_train_idx = rus.fit_resample(X_train_scaled, y_train_idx)
                ...
                print(f"\nTotal training samples after RandomUnderSampler: {len(y_train_idx):,}")
            except Exception as e:
                print(f"\nWarning: RandomUnderSampler failed with error: {e}")
                print("Continuing without RandomUnderSampler...")
        else:
            print("\nRandomUnderSampler: DISABLED")
        ...
        # Generate report
        print("\n" + "=" * 70)
        print("BASELINE RESULTS (WITH RandomUnderSampler)" if self.cfg.use_rus else "BASELINE RESULTS")
        print("=" * 70)
        ...
        # Save report
        suffix = "_rus" if self.cfg.use_rus else ""
...
def main():
    ...
    cfg = BaselineConfig(
        ...
        batch_size=128,
        epochs=100,
        use_rus=True,  # Enable RandomUnderSampler
        rus_sampling_strategy="auto",  # Balance all classes by undersampling majority
    ...
    print("Enhanced with RandomUnderSampler")
    print("=" * 70)
