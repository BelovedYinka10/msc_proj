...
# Enhanced with ClusterCentroids for handling class imbalance
...
class BaselineConfig:
    ...
    use_cc: bool = True  # Set to False to disable ClusterCentroids
    cc_sampling_strategy: str = "auto"  # 'auto', 'majority', 'not minority', 'all', or dict
    cc_voting: str = "auto"  # 'auto' or 'hard' - how to compute cluster centroids

    # Class weighting options
    use_class_weight: bool = True  # ClusterCentroids is undersampling, so class weights may still help
...
class BaselineIDS:
    ...
    def train_and_evaluate(self, X: pd.DataFrame, y: pd.Series):
        ...
        print("\nClass distribution (BEFORE ClusterCentroids):")
        ...
        # Apply ClusterCentroids if enabled
        if self.cfg.use_cc:
            print(f"\nApplying ClusterCentroids...")
            print(f"  Sampling strategy: {self.cfg.cc_sampling_strategy}")
            print(f"  Voting: {self.cfg.cc_voting}")
            ...
            cc = ClusterCentroids(
                sampling_strategy=self.cfg.cc_sampling_strategy,
                voting=self.cfg.cc_voting,
                random_state=self.cfg.random_state
            )

            try:
                X_train_scaled, y_train_idx = cc.fit_resample(X_train_scaled, y_train_idx)
                ...
                print(f"\nTotal training samples after ClusterCentroids: {len(y_train_idx):,}")
            except Exception as e:
                print(f"\nWarning: ClusterCentroids failed with error: {e}")
                print("Continuing without ClusterCentroids...")
        else:
            print("\nClusterCentroids: DISABLED")
        ...
        # Generate report
        print("\n" + "=" * 70)
        print("BASELINE RESULTS (WITH ClusterCentroids)" if self.cfg.use_cc else "BASELINE RESULTS")
        print("=" * 70)
        ...
        # Save report
        suffix = "_cc" if self.cfg.use_cc else ""
...
def main():
    ...
    cfg = BaselineConfig(
        ...
        batch_size=128,
        epochs=100,
        use_cc=True,  # Enable ClusterCentroids
        cc_sampling_strategy="auto",  # Balance all classes by undersampling
        cc_voting="auto",  # Auto voting for cluster centroids
    ...
    print("Enhanced with ClusterCentroids Undersampling")
    print("=" * 70)
