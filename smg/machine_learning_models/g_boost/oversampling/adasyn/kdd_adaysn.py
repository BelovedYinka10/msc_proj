...
# CHANGED: use imblearn's Pipeline so we can insert ADASYN
...
# -------- ADASYN sampler --------
# You can tune n_neighbors or sampling_strategy if needed.
adasyn = ADASYN(random_state=42, n_neighbors=5)

# -------- build pipeline (prep -> ADASYN -> clf) --------
pipe = Pipeline(steps=[
    ("prep", pre),
    ("adasyn", adasyn),  # oversample MINORITY classes on the fly during fit
    ("clf", GradientBoostingClassifier(random_state=42, **gb_params)),
])

# -------- train & predict --------
print("\n⏳ Training (5-class with GradientBoosting + ADASYN)...")
t0 = time.time();
...
plt.title("Gradient Boosting + ADASYN — NSL-KDD 5-class (pre-grouped CSVs)")
plt.tight_layout()
plt.show()
