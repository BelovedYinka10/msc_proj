...
# Now with ClusterCentroids undersampling inserted into the pipeline.
...
# NEW: ClusterCentroids (undersampling)
...
BEST_JSON = "/Users/mac/Desktop/machine learning/smg/machine_learning_models/g_boost/kdd_best_gb_hyperparameters.json"
...
# Default sampling_strategy="auto" reduces ALL majority classes down to the MINORITY count.
# That can be aggressive; if you want a gentler reduction, we can pass a dict.
cc = ClusterCentroids(random_state=42)

# -------- build pipeline (prep -> ClusterCentroids -> clf) --------
pipe = Pipeline(steps=[
    ("prep", pre),
    ("cc", cc),
...
# -------- train & predict --------
print("\n⏳ Training (5-class with GradientBoosting + ClusterCentroids)...")
...
plt.title("Gradient Boosting + ClusterCentroids — NSL-KDD 5-class (pre-grouped CSVs)")
plt.tight_layout()
plt.show()
