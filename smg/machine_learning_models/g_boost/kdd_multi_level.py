...
# ⬇️ CHANGED: use GradientBoosting instead of DecisionTree
...
# ⬇️ CHANGED: point to your GB best-params JSON
PARAMS_JSON = "/Users/mac/Desktop/machine learning/smg/machine_learning_models/g_boost/kdd_best_gb_hyperparameters.json"
...
# =======================
# Load tuned GB params (kept function name to preserve structure)
# =======================
def load_best_dt_params(json_path):
    ...
    try:
        ...
        # ⬇️ CHANGED: validate against GradientBoostingClassifier params
        valid = GradientBoostingClassifier().get_params().keys()
...
print("\n=== Stage 1: Gradient Boosting Multi-class (all classes) ===")
model_multi = GradientBoostingClassifier(**dt_params)
t0 = time.time()
...
if passed1:
    ...
    print("\n=== Stage 2: Gradient Boosting Binary (Normal vs Attack) ===")
    model_bin = GradientBoostingClassifier(**dt_params)
    t0 = time.time()
    ...
    if not passed2:
        ...
        print("\n=== Stage 3: Gradient Boosting Multi-class (attacks only) ===")
        model_attack = GradientBoostingClassifier(**dt_params)
        t0 = time.time()
        ...
        acc3, metrics3, passed3, cm3 = evaluate_multiclass(y_attack_test_m, y_pred_attack, attack_class_names,
                                                           f_min=F_MIN)
        ...
        if passed3:
            ...
            while len(np.unique(y_curr_train)) > 2 and len(y_curr_test) > 0:
                ...
                print("\n=== Stage 4: Gradient Boosting Binary (picked vs others) ===")
                model_bin_rec = GradientBoostingClassifier(**dt_params)
                t0 = time.time()
                ...
                acc_b, metrics_b, passed_b, cm_b = evaluate_binary(y_bin_test, y_pred_b, f_min=F_MIN,
                                                                   labels=(pick_name, "Others"))
            ...
            if len(np.unique(y_curr_train)) == 2 and len(y_curr_test) > 0:
                ...
                print("\n=== Stage 4 Final: Gradient Boosting Binary (last two attacks) ===")
                model_final = GradientBoostingClassifier(**dt_params)
                t0 = time.time()
