...
# Split training data among K clients (IID assumption for simulation)
client_data_splits = []
...
# --- Federated Training (Random Forest Ensemble Simulation) ---
# For Random Forest, FedAvg (averaging weights) is not directly applicable.
# This simulation implements a Federated Ensemble approach:
# Clients train local RF models, and the server aggregates their predictions via majority voting.
...
print(f"Starting Federated Random Forest Ensemble training with {K} clients and {R} rounds.")
...
for r in range(R):
    ...
    for k in range(K):
        ...
        # Using the same parameters as the original centralized script
        client_rf = RandomForestClassifier(n_estimators=100, random_state=42)
        client_rf.fit(X_client, y_client)
        trained_models.append(client_rf)
    ...
    def majority_vote(predictions_row):
        ...
        # argmax returns the index of the maximum count, which is the mode
        if len(predictions_row) == 0:
            return -1  # Or handle as appropriate for your class labels
        counts = np.bincount(predictions_row)
...
# This is the actual testing/inference step for the final aggregated model
predictions = []
for model in federated_ensemble:
    predictions.append(model.predict(X_test))
...
print("\n=== Federated Random Forest Ensemble Metrics ===")
...
# Use metrics from the last round (already calculated and stored in round_metrics)
...
plt.title('Federated Random Forest Ensemble Performance Over Rounds')
