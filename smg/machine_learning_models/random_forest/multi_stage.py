...
JSON_PATH = "best_rf_hyperparameters.json"   # adjust if saved elsewhere
...
# -----------------------
# 2.1 Load saved hyperparameters (fallback to your inline dict)
# -----------------------
fallback_params = {
    "min_samples_split": 5,
    "min_samples_leaf": 2,
...
rf_params = fallback_params.copy()
try:
    ...
    rf_params.update(saved_params)
    print(f"\n🔧 Loaded RandomForest params from '{JSON_PATH}': {rf_params}")
except FileNotFoundError:
    print(f"\nℹ️ '{JSON_PATH}' not found. Using fallback params: {rf_params}")
except Exception as e:
    print(f"\n⚠️ Could not load '{JSON_PATH}' ({e}). Using fallback params: {rf_params}")

# always enforce these to match your previous setup
rf_params.update({"class_weight": "balanced", "random_state": 42})
...
print("\n=== Stage 1: Random Forest Multi-class ===")
model_multi = RandomForestClassifier(**rf_params)
t0 = time.time()
...
if passed:
    ...
    model_bin = RandomForestClassifier(**rf_params)
    t0 = time.time()
    ...
    if passed_bin:
        ...
        print("\n=== Stage 3: Random Forest Multi-class on Attacks ===")
        model_attack = RandomForestClassifier(**rf_params)
        t0 = time.time()
        ...
        if passed_att:
            ...
            while len(np.unique(y_attack_train_idx)) > 2:
                ...
                model_bin_rec = RandomForestClassifier(**rf_params)
                t0 = time.time()
...
# -----------------------
print("\n" + "="*60)
print("⏱ TOTAL RUNTIME SUMMARY (excludes hyperparam search)")
print("="*60)
