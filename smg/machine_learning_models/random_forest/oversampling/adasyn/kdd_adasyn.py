# kdd_benchmark_5class_rf_adasyn.py
# Uses pre-made 5-class CSVs and PRINTS metrics only (loads saved hyperparameters).
# Now with ADASYN oversampling inserted into the pipeline.
...
# Use imblearn's Pipeline so we can insert ADASYN
...
# -------- preprocessing --------
try:
    ohe = OneHotEncoder(handle_unknown="ignore", sparse_output=False)  # dense for ADASYN
except TypeError:  # older sklearn
    ohe = OneHotEncoder(handle_unknown="ignore", sparse=False)  # dense for ADASYN
...
# -------- ADASYN sampler --------
adasyn = ADASYN(random_state=42, n_neighbors=5)  # tune n_neighbors/sampling_strategy if desired

# -------- build pipeline (prep -> ADASYN -> clf) --------
pipe = Pipeline(steps=[
    ("prep", pre),
    ("adasyn", adasyn),
...
# -------- train & predict --------
print("\n⏳ Training (5-class with Random Forest + ADASYN)...")
t0 = time.time();
...
plt.title("Random Forest + ADASYN — NSL-KDD 5-class (pre-grouped CSVs)")
plt.tight_layout()
plt.show()
