# kdd_benchmark_5class_rf_ros.py
# Uses pre-made 5-class CSVs and PRINTS metrics only (loads saved hyperparameters).
# Now with RandomOverSampler (ROS) inserted into the pipeline.
...
# -------- preprocessing --------
try:
    ohe = OneHotEncoder(handle_unknown="ignore", sparse_output=False)  # dense works fine for ROS
...
# Default 'sampling_strategy="auto"' raises all minority classes up to the majority class size.
ros = RandomOverSampler(random_state=42)

# -------- build pipeline (prep -> ROS -> clf) --------
pipe = Pipeline(steps=[
    ("prep", pre),
    ("ros", ros),
...
# -------- train & predict --------
print("\n⏳ Training (5-class with Random Forest + ROS)...")
...
plt.title("Random Forest + ROS — NSL-KDD 5-class (pre-grouped CSVs)")
plt.tight_layout()
plt.show()
