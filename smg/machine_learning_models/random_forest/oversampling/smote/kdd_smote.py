# kdd_benchmark_5class_rf_smote.py
# Uses pre-made 5-class CSVs and PRINTS metrics only (loads saved hyperparameters).
# Now with SMOTE oversampling inserted into the pipeline.
...
# -------- preprocessing --------
try:
    ohe = OneHotEncoder(handle_unknown="ignore", sparse_output=False)  # dense for SMOTE
except TypeError:  # older sklearn
    ohe = OneHotEncoder(handle_unknown="ignore", sparse=False)         # dense for SMOTE
...
# -------- SMOTE sampler --------
# If you hit "not enough neighbors" for tiny classes, try k_neighbors=3.
smote = SMOTE(random_state=42, k_neighbors=5)

# -------- build pipeline (prep -> SMOTE -> clf) --------
pipe = Pipeline(steps=[
    ("prep", pre),
    ("smote", smote),
...
# -------- train & predict --------
print("\n⏳ Training (5-class with Random Forest + SMOTE)...")
...
plt.title("Random Forest + SMOTE — NSL-KDD 5-class (pre-grouped CSVs)")
