# kdd_benchmark_5class_rf_borderline.py
...
# -------- preprocessing --------
try:
    ohe = OneHotEncoder(handle_unknown="ignore", sparse_output=False)  # dense is fine for SMOTE variants
...
# Defaults to 'borderline-1'. To try the more aggressive variant, set kind='borderline-2'.
blsmote = BorderlineSMOTE(random_state=42, k_neighbors=5, m_neighbors=10)  # kind='borderline-1' by default
...
# -------- train & predict --------
print("\n⏳ Training (5-class with Random Forest + Borderline-SMOTE)...")
...
plt.title("Random Forest + Borderline-SMOTE — NSL-KDD 5-class (pre-grouped CSVs)")
plt.tight_layout()
plt.show()
