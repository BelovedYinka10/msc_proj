# nsl_kdd_rf_randomsearch.py
# Hyperparameter search for Random Forest on NSL-KDD (5-class pre-grouped CSVs)
...
from sklearn.ensemble import RandomForestClassifier  # ⬅️ changed
...
param_dist = {
    "clf__n_estimators": [100, 200, 400],      # 3 options
    "clf__max_depth": [None, 10, 20],          # 3 options
    "clf__min_samples_split": [2, 10],         # 2 options
    "clf__min_samples_leaf": [1, 4],           # 2 options
    "clf__max_features": ["sqrt", "log2"],     # 2 options
    "clf__bootstrap": [True, False],           # 2 options
    # Optional (comment in if you want imbalance-aware RF)
...
print("\n⏳ Starting RandomizedSearchCV (Random Forest on NSL-KDD 5-class)...")
print("⚡ ULTRA SPEED MODE: 10 iterations × 2 folds = 20 fits total")
t0 = time.time()
rand = RandomizedSearchCV(
    estimator=pipe,
    param_distributions=param_dist,
    n_iter=10,                 # ultra fast
    scoring=scorer,
    cv=cv,
    n_jobs=-1,
    verbose=2,
...
print("\n=== Randomized Search Results (Random Forest) ===")
...
with open("kdd_best_rf_hyperparameters.json", "w") as f:
    json.dump(out, f, indent=2)

print("✅ Saved best params to 'kdd_best_rf_hyperparameters.json'")
