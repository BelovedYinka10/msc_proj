# kdd_benchmark_5class_rf_nearmiss.py
# Uses pre-made 5-class CSVs and PRINTS metrics only (loads saved hyperparameters).
# Now with NearMiss under-sampling inserted into the pipeline.
...
BEST_JSON = "/Users/mac/Desktop/machine learning/smg/machine_learning_models/random_forest/kdd_best_rf_hyperparameters.json"  # from nsl_kdd_rf_randomsearch.py
...
# -------- preprocessing --------
try:
    ohe = OneHotEncoder(handle_unknown="ignore", sparse_output=False)  # dense works fine for NearMiss
...
try:
    ...
    # fallback defaults for RandomForest
...
# Filter params to valid keys for RandomForestClassifier
valid_keys = RandomForestClassifier().get_params().keys()
rf_params = {k: v for k, v in best_params_loaded.items() if k in valid_keys}

# -------- NearMiss under-sampler --------
# version=1: select majority samples whose average distance to the 3 closest minority samples is the smallest.
# By default, balances all classes down to the minority size (be careful with extreme imbalance).
nm = NearMiss(version=1, n_neighbors=3)

# -------- build pipeline (prep -> NearMiss -> clf) --------
pipe = Pipeline(steps=[
    ("prep", pre),
    ("nearmiss", nm),
...
# -------- train & predict --------
print("\n⏳ Training (5-class with Random Forest + NearMiss)...")
...
plt.title("Random Forest + NearMiss — NSL-KDD 5-class (pre-grouped CSVs)")
plt.tight_layout()
plt.show()
