...
print("🚀 Random Forest with TomekLinks Sampling - Optimized Workflow")
print("="*70)
...
# Apply TomekLinks to training data ONLY
try:
    X_train_resampled, y_train_resampled = tl.fit_resample(X_train, y_train)
    
    print(f"✅ TomekLinks sampling completed successfully!")
    ...
    for class_idx, count in sorted(new_counts.items()):
        ...
        print(f"   Class {class_names[class_idx]}: {count} samples ({percentage:.1f}%) [-{reduction} removed]")
...
print(f"\n⏳ Training Random Forest model on TomekLinks-resampled data...")
...
print("\n" + "="*60)
print("🎯 RANDOM FOREST + TOMEKLINKS RESULTS")
print("="*60)
...
fig.update_layout(
    title={
        'text': 'Confusion Matrix - Random Forest + TomekLinks',
        'x': 0.5,
        'xanchor': 'center',
...
print(f"   - Random Forest + TomekLinks (this experiment)")
print(f"   - XGBoost + TomekLinks (next?)")
print(f"   - Decision Tree + TomekLinks (next?)")

print(f"\n🎉 Random Forest + TomekLinks experiment completed successfully!")
