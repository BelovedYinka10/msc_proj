# kdd_benchmark_5class_rf_tomek.py
# Uses pre-made 5-class CSVs and PRINTS metrics only (loads saved hyperparameters).
# Now with Tomek Links (under-sampling/cleaning) inserted into the pipeline.
...
# NEW: Tomek Links under-sampler (cleaning method)
...
BEST_JSON = "/Users/mac/Desktop/machine learning/smg/machine_learning_models/random_forest/kdd_best_rf_hyperparameters.json"
...
# -------- preprocessing --------
try:
    ohe = OneHotEncoder(handle_unknown="ignore", sparse_output=False)  # dense works fine for Tomek Links (distance-based)
...
# Removes majority samples that form Tomek pairs with a minority neighbor (reduces class overlap).
tomek = TomekLinks(sampling_strategy="auto")

# -------- build pipeline (prep -> TomekLinks -> clf) --------
pipe = Pipeline(steps=[
    ("prep", pre),
    ("tomek", tomek),
...
# -------- train & predict --------
print("\n⏳ Training (5-class with Random Forest + Tomek Links)...")
...
plt.title("Random Forest + Tomek Links — NSL-KDD 5-class (pre-grouped CSVs)")
plt.tight_layout()
plt.show()
