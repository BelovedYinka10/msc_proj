# kdd_benchmark_5class_rf_rus.py
# Uses pre-made 5-class CSVs and PRINTS metrics only (loads saved hyperparameters).
# Now with RandomUnderSampler (RUS) inserted into the pipeline.
...
# -------- preprocessing --------
try:
    ohe = OneHotEncoder(handle_unknown="ignore", sparse_output=False)  # dense works fine for RUS
...
# Default 'sampling_strategy="auto"' downsamples every majority class to the minority size.
rus = RandomUnderSampler(random_state=42)

# -------- build pipeline (prep -> RUS -> clf) --------
pipe = Pipeline(steps=[
    ("prep", pre),
    ("rus", rus),
...
# -------- train & predict --------
print("\n⏳ Training (5-class with Random Forest + RUS)...")
...
plt.title("Random Forest + RUS — NSL-KDD 5-class (pre-grouped CSVs)")
plt.tight_layout()
plt.show()
