# kdd_benchmark_5class_rf_cc.py
# Uses pre-made 5-class CSVs and PRINTS metrics only (loads saved hyperparameters).
# Now with Cluster Centroids (under-sampling) inserted into the pipeline.
...
# NEW: Cluster Centroids under-sampler
...
# -------- preprocessing --------
try:
    ohe = OneHotEncoder(handle_unknown="ignore", sparse_output=False)  # dense is fine
...
# Under-samples majority classes by replacing clusters with their centroids.
cc = ClusterCentroids(random_state=42)

# -------- build pipeline (prep -> CC -> clf) --------
pipe = Pipeline(steps=[
    ("prep", pre),
    ("cc", cc),
...
# -------- train & predict --------
print("\n⏳ Training (5-class with Random Forest + Cluster Centroids)...")
t0 = time.time();
...
plt.title("Random Forest + Cluster Centroids — NSL-KDD 5-class (pre-grouped CSVs)")
plt.tight_layout()
plt.show()
