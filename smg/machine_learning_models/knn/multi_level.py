...
JSON_PATH = "best_knn_hyperparameters.json"   # adjust path if stored elsewhere
...
# -----------------------
F_MIN = 0.88
K_NEIGHBORS = 5  # default if JSON not present

# Default KNN params; will be overridden by JSON if available
DEFAULT_KNN_PARAMS = {
    ...
    "algorithm": "auto",
    "leaf_size": 30,
    # "n_jobs": -1,  # include if your sklearn version supports it
}

# Load saved best hyperparameters (if present)
knn_params = DEFAULT_KNN_PARAMS.copy()
try:
    with open(JSON_PATH, "r") as f:
        saved = json.load(f)
    saved_params = saved.get("best_parameters", {})
    valid_keys = KNeighborsClassifier().get_params().keys()
    saved_params = {k: v for k, v in saved_params.items() if k in valid_keys}
    knn_params.update(saved_params)
    print(f"\n🔧 Loaded KNN params from '{JSON_PATH}':\n{knn_params}")
except FileNotFoundError:
    print(f"\nℹ️ '{JSON_PATH}' not found. Using default KNN params:\n{knn_params}")
except Exception as e:
    print(f"\n⚠️ Could not load '{JSON_PATH}' ({e}). Using default KNN params:\n{knn_params}")

# -----------------------
# 3. Evaluation Functions (unchanged)
# -----------------------
...
print("\n=== Stage 1: KNN Multi-class ===")
model_multi = KNeighborsClassifier(**knn_params)
...
if passed:
    ...
    model_bin = KNeighborsClassifier(**knn_params)
    ...
    if passed_bin:
        ...
        print("\n=== Stage 3: KNN Multi-class on Attacks ===")
        model_attack = KNeighborsClassifier(**knn_params)
        ...
        if passed_att:
            ...
            while len(np.unique(y_attack_train_idx)) > 2:
                ...
                model_bin_rec = KNeighborsClassifier(**knn_params)
            ...
            if len(np.unique(y_attack_train_idx)) == 2:
                ...
                model_final = KNeighborsClassifier(**knn_params)
