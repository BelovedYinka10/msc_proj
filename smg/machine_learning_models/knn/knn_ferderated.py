...
# --- Federated Training (KNN Ensemble Simulation) ---
# KNN is a lazy learner; training involves storing data. We simulate FL via 
# aggregating predictions from local KNN models over rounds.
...
# KNN parameters from the centralized script (defaulted for this example)
KNN_PARAMS = {'n_neighbors': 5, 'weights': 'distance', 'metric': 'euclidean'}

print(f"Starting Federated KNN Ensemble simulation with {K} clients and {R} rounds.")

for r in range(R):
    print(f"\n--- Communication Round {r+1} of {R} ---")
    
    # Clients "train" (fit) local KNN models
    trained_models = []
    for k in range(K):
        X_client, y_client = client_data_splits[k]
        
        # Instantiate and train a KNN classifier
        client_knn = KNeighborsClassifier(**KNN_PARAMS)
        client_knn.fit(X_client, y_client)
        trained_models.append(client_knn)
...
print("\n=== Federated KNN Ensemble Metrics ===")
...
plt.title('Federated KNN Ensemble Performance Over Rounds')
