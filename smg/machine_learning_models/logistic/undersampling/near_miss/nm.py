...
print("🚀 Logistic Regression with NearMiss - Optimized Workflow")
print("="*70)
...
print(f"\n🎲 Applying NearMiss Sampling...")

# Initialize NearMiss - REMOVED random_state
near_miss = NearMiss(
...
try:
    ...
    for class_idx, count in sorted(new_counts.items()):
        ...
        if change < 0:
            print(f"   Class {class_names[class_idx]}: {count} samples ({percentage:.1f}%) [{change} selected by distance]")
        else:
            print(f"   Class {class_names[class_idx]}: {count} samples ({percentage:.1f}%) [unchanged]")
    
except ValueError as e:
    print(f"❌ NearMiss sampling failed: {e}")
...
print(f"\n⏳ Training Logistic Regression model on NearMiss-resampled data...")
...
print("\n" + "="*60)
print("🎯 LOGISTIC REGRESSION + NEARMISS RESULTS")
print("="*60)
...
fig.update_layout(
    title={
        'text': 'Confusion Matrix - Logistic Regression + NearMiss',
        'x': 0.5,
        'xanchor': 'center',
...
print(f"   - XGBoost + NearMiss (next?)")

print(f"\n🎉 Logistic Regression + NearMiss experiment completed successfully!")