...
print("🚀 Logistic Regression with Tomek Links - Optimized Workflow")
print("="*70)
...
tomek_links = TomekLinks(
    sampling_strategy='all', # Removes both majority and minority samples in a Tomek Link
    n_jobs=-1
)

# Apply Tomek Links to training data ONLY
try:
    X_train_resampled, y_train_resampled = tomek_links.fit_resample(X_train, y_train)
    
    print(f"✅ Tomek Links completed successfully!")
    print(f"   Original training samples: {X_train.shape[0]}")
    print(f"   Resampled training samples: {X_train_resampled.shape[0]}")
    
    # Show new class distribution
    print(f"\n📈 New Class Distribution (After Tomek Links):")
    ...
    for class_idx, count in sorted(new_counts.items()):
        ...
        print(f"   Class {class_names[class_idx]}: {count} samples ({percentage:.1f}%) [{change} removed]")
    
except ValueError as e:
    print(f"❌ Tomek Links failed: {e}")
...
print(f"\n⏳ Training Logistic Regression model on Tomek-resampled data...")
...
print("\n" + "="*60)
print("🎯 LOGISTIC REGRESSION + TOMEK LINKS RESULTS")
print("="*60)
...
fig.update_layout(
    title={
        'text': 'Confusion Matrix - Logistic Regression + Tomek Links',
        'x': 0.5,
        'xanchor': 'center',
...
print(f"   - Logistic Regression + NearMiss (undersampling)")
print(f"   - Logistic Regression + RUS (undersampling)")
print(f"   - Logistic Regression + Tomek Links (undersampling)")

print(f"\n🎉 Logistic Regression + Tomek Links experiment completed successfully!")