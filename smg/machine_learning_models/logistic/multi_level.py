...
JSON_PATH = "best_lr_hyperparameters.json"   # change if your file is elsewhere
...
# -----------------------
# 2.1) Load saved hyperparameters (fallback to your original settings)
# -----------------------
# Defaults (match your original architecture)
default_multi = dict(max_iter=500, multi_class="multinomial", solver="lbfgs", random_state=42)
default_binary = dict(max_iter=500, solver="lbfgs", random_state=42)

lr_params_multi  = default_multi.copy()
lr_params_binary = default_binary.copy()
...
try:
    ...
    # keep only valid sklearn params
    saved_params = {k: v for k, v in saved_params.items() if k in valid_keys}

    # use the same params for both, but
    # Stage 1 & 3 use multinomial, Stage 2 & 4 (binary) do not need multi_class
    lr_params_multi.update(saved_params)

    # for binary, copy but drop multi_class if present
    temp = saved_params.copy()
    temp.pop("multi_class", None)
    lr_params_binary.update(temp)

    print(f"\n🔧 Loaded LogisticRegression params from '{JSON_PATH}':")
    print(f"  multi-class params: {lr_params_multi}")
    print(f"  binary params     : {lr_params_binary}")
except FileNotFoundError:
    print(f"\nℹ️ '{JSON_PATH}' not found. Using defaults:")
    print(f"  multi-class params: {lr_params_multi}")
    print(f"  binary params     : {lr_params_binary}")
...
print("\n=== Stage 1: Logistic Regression Multi-class ===")
model_multi = LogisticRegression(**lr_params_multi)
...
if passed:
    ...
    model_bin = LogisticRegression(**lr_params_binary)
    ...
    if passed_bin:
        ...
        print("\n=== Stage 3: Logistic Regression Multi-class on Attacks ===")
        model_attack = LogisticRegression(**lr_params_multi)
        ...
        if passed_att:
            ...
            while len(np.unique(y_attack_train_idx)) > 2:
                ...
                model_bin_rec = LogisticRegression(**lr_params_binary)
            ...
            if len(np.unique(y_attack_train_idx)) == 2:
                ...
                model_final = LogisticRegression(**lr_params_binary)
...
# -----------------------
# 5) Total runtime summary (final fit/predict only)
# -----------------------
print("\n" + "="*60)
