...
print("🚀 Logistic Regression with ROS (Random OverSampler) - Optimized Workflow")
print("="*70)
...
# ============================================
# APPLY ROS (RANDOM OVERSAMPLER) SAMPLING
# ============================================

print(f"\n🎲 Applying ROS (Random OverSampler) Sampling...")
...
try:
    ...
    for class_idx, count in sorted(new_counts.items()):
        ...
        print(f"   Class {class_names[class_idx]}: {count} samples ({percentage:.1f}%) [+{increase} duplicated]")
    
except ValueError as e:
    print(f"❌ ROS sampling failed: {e}")
...
print(f"\n⏳ Training Logistic Regression model on ROS-resampled data...")
...
print("\n" + "="*60)
print("🎯 LOGISTIC REGRESSION + ROS RESULTS")
print("="*60)
...
fig.update_layout(
    title={
        'text': 'Confusion Matrix - Logistic Regression + ROS',
        'x': 0.5,
        'xanchor': 'center',
...
print(f"🎲 Sampling: ROS (Random OverSampler)")
...
print(f"🔍 Duplicated Samples Added: {X_train_resampled.shape[0] - X_train.shape[0]}")
...
print(f"   - Random Forest + ROS (previous experiment)")
print(f"   - Logistic Regression + ROS (this experiment)")
print(f"   - XGBoost + ROS (next?)")

print(f"\n🎉 Logistic Regression + ROS experiment completed successfully!")