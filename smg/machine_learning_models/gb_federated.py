...
# --- Data Loading ---
# NOTE: This script assumes the dataset 'dataset_EPICA_raw 1.csv' is available
# at the specified path "../../EPIC/dataset_EPICA_raw 1.csv" when executed.
try:
    ...
    # Exit or handle error if the dataset is not found during execution
    exit()
...
# Train-test split (Used for FL client data splitting and the global test set)
...
# Split training data among K clients (IID simulation)
client_data_splits = []
...
# --- Federated Training (Gradient Boosting Ensemble Simulation) ---
# We use a Federated Ensemble approach: clients train local GB models, and the server 
...
# This list stores all models trained across all rounds
federated_ensemble = []

print(f"Starting Federated Gradient Boosting Ensemble training with {K} clients and {R} rounds.")

for r in range(R):
    print(f"\n--- Communication Round {r+1} of {R} ---")
    
    # Clients train local Gradient Boosting models
    trained_models = []
    for k in range(K):
        X_client, y_client = client_data_splits[k]
        
        # Instantiate and train a Gradient Boosting Classifier
        # Default parameters often used in centralized setup
        client_gb = GradientBoostingClassifier(n_estimators=100, learning_rate=0.1, max_depth=3, random_state=42)
        client_gb.fit(X_client, y_client)
        trained_models.append(client_gb)
    ...
    def majority_vote(predictions_row):
        if len(predictions_row) == 0:
            return -1 
        counts = np.bincount(predictions_row[predictions_row >= 0])
...
# Calculate Testing Time (Time for the final aggregation and prediction on the test set)
start_test = time.time()

# Final prediction step (same as inside the loop, just timing it separately)
predictions = []
for model in federated_ensemble:
    predictions.append(model.predict(X_test))

predictions_array = np.array(predictions).T
y_pred = np.apply_along_axis(majority_vote, axis=1, arr=predictions_array)
end_test = time.time()

print("\n=== Federated Gradient Boosting Ensemble Metrics ===")
...
# Use metrics from the last round
...
plt.title('Federated Gradient Boosting Ensemble Performance Over Rounds')
