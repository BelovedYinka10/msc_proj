...
# Uses pre-made 5-class CSVs and PRINTS metrics only (no file saves).
...
BEST_PARAMS_JSON = "best_dt_hyperparameters.json"

def load_df(p: Path) -> pd.DataFrame:
    df = pd.read_csv(p)
    df.columns = [c.strip() for c in df.columns]
    if TARGET_COL not in df.columns:
        raise ValueError(f"{Path(p).name} missing '{TARGET_COL}'. Columns: {df.columns.tolist()}")
    return df
...
print(f"\n🔎 Loading best hyperparameters from '{BEST_PARAMS_JSON}'...")
try:
    with open(BEST_PARAMS_JSON, "r") as f:
        best_params = json.load(f)["best_parameters"]
    print("Using saved hyperparameters:", best_params)
except Exception as e:
    print(f"⚠️  Could not load '{BEST_PARAMS_JSON}' ({e}). Falling back to defaults.")
    best_params = {"criterion":"entropy","max_depth":25,"min_samples_split":15,
                   "min_samples_leaf":8,"max_leaf_nodes":200}

# Ensure class_weight is set (helps imbalance)
if best_params.get("class_weight") in (None, "none"):
...
# ---- NEW: format as percentages ----
for col in ["precision", "recall", "f1", "accuracy_ovr", "specificity"]:
...
plt.title("Decision Tree Baseline — NSL-KDD 5-class")
plt.tight_layout()
plt.show()
