...
# Uses pre-made 5-class CSVs, TomekLinks sampling, and PRINTS metrics only (no file saves).
...
# -------- Preprocessing and TomekLinks Sampling --------
...
# Apply TomekLinks sampling
print("\n⏳ Applying TomekLinks sampling...")
tomek_start = time.time()
tomek = TomekLinks(n_jobs=-1)
X_train_resampled, y_train_resampled = tomek.fit_resample(X_train_prep, y_train)
tomek_time = time.time() - tomek_start

print(f"✅ TomekLinks sampling completed in {tomek_time:.3f}s")
...
# Since we're using RUS, we don't need class_weight balancing
if 'class_weight' in best_params:
    best_params.pop('class_weight')
    print("   Removed 'class_weight' parameter since RUS handles class imbalance")

clf = DecisionTreeClassifier(random_state=42, **best_params)

# -------- train & predict --------
print("\n⏳ Training (5-class with RUS)...")
t0 = time.time();
...
plt.title("Decision Tree — NSL-KDD 5-class (RUS + Optimized Hyperparameters)")
plt.tight_layout()
plt.show()
