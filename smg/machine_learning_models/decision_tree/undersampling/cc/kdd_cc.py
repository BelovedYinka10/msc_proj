...
# Uses pre-made 5-class CSVs, ClusterCentroids sampling, and PRINTS metrics only (no file saves).
...
# -------- Preprocessing and ClusterCentroids Undersampling --------
...
# Apply ClusterCentroids undersampling
print("\n⏳ Applying ClusterCentroids undersampling...")
cc_start = time.time()
cc = ClusterCentroids(random_state=42)
X_train_resampled, y_train_resampled = cc.fit_resample(X_train_prep, y_train)
cc_time = time.time() - cc_start

print(f"✅ ClusterCentroids undersampling completed in {cc_time:.3f}s")
...
# Since we're using ClusterCentroids, we don't need class_weight balancing
if 'class_weight' in best_params:
    best_params.pop('class_weight')
    print("   Removed 'class_weight' parameter since ClusterCentroids handles class imbalance")

clf = DecisionTreeClassifier(random_state=42, **best_params)

# -------- train & predict --------
print("\n⏳ Training (5-class with ClusterCentroids)...")
t0 = time.time();
...
plt.title("Decision Tree — NSL-KDD 5-class (ClusterCentroids + Optimized Hyperparameters)")
plt.tight_layout()
plt.show()
