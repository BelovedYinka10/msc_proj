...
# Uses pre-made 5-class CSVs, ROS (Random Over Sampling), and PRINTS metrics only (no file saves).
...
# -------- Preprocessing and ROS (Random Over Sampling) --------
...
# Apply ROS (Random Over Sampling)
print("\n⏳ Applying ROS (Random Over Sampling)...")
ros_start = time.time()
ros = RandomOverSampler(random_state=42)
X_train_resampled, y_train_resampled = ros.fit_resample(X_train_prep, y_train)
ros_time = time.time() - ros_start

print(f"✅ ROS sampling completed in {ros_time:.3f}s")
...
# Since we're using ROS, we don't need class_weight balancing
if 'class_weight' in best_params:
    best_params.pop('class_weight')
    print("   Removed 'class_weight' parameter since ROS handles class imbalance")

clf = DecisionTreeClassifier(random_state=42, **best_params)

# -------- train & predict --------
print("\n⏳ Training (5-class with ROS)...")
...
plt.title("Decision Tree — NSL-KDD 5-class (ROS + Optimized Hyperparameters)")
plt.tight_layout()
plt.show()