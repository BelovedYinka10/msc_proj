...
# Uses pre-made 5-class CSVs, SMOTE sampling, and PRINTS metrics only (no file saves).
...
# -------- Preprocessing and SMOTE Sampling --------
...
print(f"✅ SMOTE sampling completed in {smote_time:.3f}s")
...
# Since we're using SMOTE, we don't need class_weight balancing
if 'class_weight' in best_params:
    best_params.pop('class_weight')
    print("   Removed 'class_weight' parameter since SMOTE handles class imbalance")

clf = DecisionTreeClassifier(random_state=42, **best_params)

# -------- train & predict --------
print("\n⏳ Training (5-class with SMOTE)...")
...
plt.title("Decision Tree — NSL-KDD 5-class (SMOTE + Optimized Hyperparameters)")
plt.tight_layout()
plt.show()