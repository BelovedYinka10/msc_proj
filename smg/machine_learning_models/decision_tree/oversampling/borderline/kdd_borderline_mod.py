...
# Uses pre-made 5-class CSVs, BorderlineSMOTE sampling, and PRINTS metrics only (no file saves).
...
# -------- Preprocessing and BorderlineSMOTE Sampling --------
...
# Apply BorderlineSMOTE sampling
print("\n⏳ Applying BorderlineSMOTE sampling...")
borderline_start = time.time()
borderline_smote = BorderlineSMOTE(random_state=42, n_jobs=-1)
X_train_resampled, y_train_resampled = borderline_smote.fit_resample(X_train_prep, y_train)
borderline_time = time.time() - borderline_start

print(f"✅ BorderlineSMOTE sampling completed in {borderline_time:.3f}s")
...
# Since we're using BorderlineSMOTE, we don't need class_weight balancing
if 'class_weight' in best_params:
    best_params.pop('class_weight')
    print("   Removed 'class_weight' parameter since BorderlineSMOTE handles class imbalance")

clf = DecisionTreeClassifier(random_state=42, **best_params)

# -------- train & predict --------
print("\n⏳ Training (5-class with ADASYN)...")
...
plt.title("Decision Tree — NSL-KDD 5-class (ADASYN + Optimized Hyperparameters)")
plt.tight_layout()
plt.show()