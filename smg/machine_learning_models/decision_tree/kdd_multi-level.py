...
PARAMS_JSON = "best_dt_hyperparameters.json"  # optional
...
print("\n=== Stage 1: Decision Tree Multi-class (all classes) ===")
...
if passed1:
    ...
    if not passed2:
        ...
        print("\n=== Stage 3: Decision Tree Multi-class (attacks only) ===")
        ...
        if passed3:
            ...
            while len(np.unique(y_curr_train)) > 2 and len(y_curr_test) > 0:
                ...
                # Train on X_curr_train, evaluate on X_curr_test
            ...
            if len(np.unique(y_curr_train)) == 2 and len(y_curr_test) > 0:
                ...
                # Use actual names in evaluate_binary (order = [name_a, name_b])
