# kdd_multi_stage_print_labels_fix_adasyn_last_stage_only.py
"""
...
CHANGED: ADASYN is applied ONLY in the final binary stage (last step of recursion).
"""
...
# --------------- ADASYN toggles ---------------
# Global (multiclass) ADASYN: DISABLED
USE_ADASYN = False
ADASYN_PARAMS = dict(
...
# Final-stage (last binary split only) ADASYN: ENABLED
USE_FINAL_ADASYN = True
FINAL_ADASYN_PARAMS = dict(
    sampling_strategy=0.80,   # bring minority to ~80% of majority (milder than full balance)
    n_neighbors=3
)
# --------------------------------------------------
...
# -----------------------
# (Global ADASYN) DISABLED
# -----------------------
if USE_ADASYN:
    try:
        from imblearn.over_sampling import ADASYN
        print("\n>>> Applying ADASYN to TRAIN set (multiclass) ...")
        print_counts("Before ADASYN (TRAIN)", y_train_enc, class_names)
        ada = ADASYN(random_state=42, **ADASYN_PARAMS)
        X_train, y_train_enc = ada.fit_resample(X_train, y_train_enc)
        print_counts("After  ADASYN (TRAIN)", y_train_enc, class_names)
    except ImportError:
        print("\n[WARN] imbalanced-learn is not installed. Skipping global ADASYN. "
              "Install with: pip install imbalanced-learn")

# One-hot encode AFTER any resampling so shapes match
...
if passed1:
    ...
    acc2, metrics2, passed2 = evaluate_binary(model_bin, X_test, y_test_bin,
                                              label_names=[normal_name, "Attack (others)"])
    ...
    if passed2:
        ...
        model_attack.fit(
            X_attack_train,
            to_categorical(y_attack_train_new, num_classes=len(attack_class_names)),
        ...
        acc3, metrics3, passed3 = evaluate_multiclass(
            model_attack, X_attack_test,
            to_categorical(y_attack_test_new, num_classes=len(attack_class_names)),
            attack_class_names
        )
        if passed3:
            ...
            while len(np.unique(labels_rec_train)) > 2:
                ...
                pick = max(cmr, key=cmr.get)  # highest CMR
                ...
                acc_rec, metrics_rec, passed_rec = evaluate_binary(
                    model_bin_rec, X_rec_test, y_bin_test,
            ...
            if len(remaining) == 2:
                pos_idx = remaining[0]; neg_idx = remaining[1]
                ...
                # ---- ADASYN ONLY HERE (final binary) ----
                X_final_train = X_rec_train
                y_final_train = y_bin_train_final
                if USE_FINAL_ADASYN:
                    try:
                        from imblearn.over_sampling import ADASYN
                        print("\n>>> Applying ADASYN in FINAL binary stage ...")
                        # mild, targeted balancing
                        ada_final = ADASYN(random_state=42, **FINAL_ADASYN_PARAMS)
                        X_final_train, y_final_train = ada_final.fit_resample(X_rec_train, y_bin_train_final)
                        # print counts with explicit label names: [neg, pos]
                        print_counts("Final stage TRAIN after ADASYN", y_final_train, [neg_name, pos_name])
                    except ImportError:
                        print("\n[WARN] imbalanced-learn is not installed. Skipping final-stage ADASYN. "
                              "Install with: pip install imbalanced-learn")
