...
for i, name in enumerate(class_names):
    ...
    f1_i = 2 * prec * rec_tpr / (prec + rec_tpr) if (prec + rec_tpr) else 0.0
    ...
    rows.append({
        ...
        "roc_auc_ovr": float(roc_auc) if roc_auc == roc_auc else None,  # None for NaN
...
print("="*50)
# Assumes class_names[0] is 'normal' — adjust if needed
normal_id = np.where(np.array([c.lower() for c in class_names]) == "normal")[0]
if len(normal_id) == 0:
    # If no explicit 'normal' label, default to index 0
    normal_id = [0]
normal_id = int(normal_id[0])
...
print("📁 Per-class CSV:   per_class_metrics.csv")
print("📁 Per-class JSON:  per_class_metrics.json")
