...
def build_model(input_dim, output_dim, binary=False, params=best_hyperparams):
    u0 = params["units_0"];
    u1 = params["units_1"]
    act0 = params["activation_0"];
    act1 = params["activation_1"]
    dr0 = params["dropout_0"];
    dr1 = params["dropout_1"]
    bn0 = params["batch_norm_0"];
    bn1 = params["batch_norm_1"]
...
if len(train_nan_idx) > 0:
    ...
    X_train_df = X_train_df.drop(index=train_nan_idx);
    y_train_ser = y_train_ser.drop(index=train_nan_idx)
...
if len(test_nan_idx) > 0:
    ...
    X_test_df = X_test_df.drop(index=test_nan_idx);
    y_test_ser = y_test_ser.drop(index=test_nan_idx)
...
if passed1:
    ...
    if passed2:
        ...
        if passed3:
            ...
            if len(remaining) == 2:
                pos_idx = remaining[0];
                neg_idx = remaining[1]  # indices in rec_class_names
                pos_name = rec_class_names[pos_idx];
                neg_name = rec_class_names[neg_idx]
