...
# NEW: Tomek Links cleaner (under-sampling by removing majority examples in TL pairs)
...
print("="*70)
print("KDD MODEL WITH BEST HYPERBAND HYPERPARAMETERS + TOMEK LINKS (CLEANING)")
print("="*70)
...
print("✅ Batch Size: 32")
print("✅ Cleaning: Tomek Links on TRAIN only (removes borderline majority points)")
print("="*70)
...
def preprocess_kdd_5class(train_data, test_data, target_col):
    ...
    # Encode categorical columns with LabelEncoder (fast + consistent)
...
# ================================
# Tomek Links cleaning (TRAIN ONLY)
# ================================
print("\n" + "="*50)
print("TOMEK LINKS CLEANING (training set only)")
print("="*50)

counts_before = Counter(y_train_enc)
print("Class counts BEFORE TomekLinks:", {class_names[k]: v for k, v in counts_before.items()})

# sampling_strategy='auto' → remove majority samples that form TL pairs w.r.t. minority classes
tl = TomekLinks(sampling_strategy='auto', n_jobs=-1)
t_us0 = time.time()
X_train_tl, y_train_tl = tl.fit_resample(X_train, y_train_enc)
t_us1 = time.time()

print(f"Tomek Links time: {t_us1 - t_us0:.2f}s")
counts_after = Counter(y_train_tl)
print("Class counts AFTER  TomekLinks:", {class_names[k]: v for k, v in counts_after.items()})
print(f"Training samples after TomekLinks: {X_train_tl.shape[0]} (from {X_train.shape[0]})")

num_classes = len(class_names)
y_train_tl_cat = tf.keras.utils.to_categorical(y_train_tl, num_classes)

# Guard: if cleaned set is tiny, skip validation split to avoid errors
val_split = 0.15 if X_train_tl.shape[0] >= 200 else 0.0
if val_split == 0.0:
    print("⚠️ Very small training set after TomekLinks → using validation_split=0.0")
...
# ================================
print("\n" + "="*50)
print("BUILDING AND TRAINING MODEL (Tomek Links)")
print("="*50)

model = create_optimal_model(X_train_tl.shape[1], len(class_names))
model.summary()

print("\n--- Training Model with Optimal Hyperparameters + Tomek Links ---")
...
per_class_df.to_csv("per_class_metrics_tomek.csv", index=False)
with open("per_class_metrics_tomek.json", "w") as f:
    json.dump(per_class_df.to_dict(orient="records"), f, indent=2)

print("\nSaved per-class metrics to:")
print("  • per_class_metrics_tomek.csv")
print("  • per_class_metrics_tomek.json")
...
# ================================
model.save('kdd_optimal_model_tomek.keras')

results_summary = {
    "timestamp": datetime.now().isoformat(),
    "model_type": "Optimal Hyperband Model + TomekLinks",
    "hyperparameters": {
        **best_hyperparams,
        "tomek_strategy": "auto (remove majority samples in TL pairs)"
    },
...
with open("optimal_model_results_tomek.json", 'w') as f:
    json.dump(results_summary, f, indent=4)

print("\n" + "="*60)
print("✅ MODEL TRAINING COMPLETE (TOMEK LINKS)!")
print("="*60)
print(f"🎯 Final Accuracy: {acc*100:.2f}%")
print("📁 Model saved as: kdd_optimal_model_tomek.keras")
print("📁 Results saved as: optimal_model_results_tomek.json")
print("📁 Per-class CSV:   per_class_metrics_tomek.csv")
print("📁 Per-class JSON:  per_class_metrics_tomek.json")
