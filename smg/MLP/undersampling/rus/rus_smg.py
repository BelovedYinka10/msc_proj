...
print("="*70)
print("KDD MODEL WITH BEST HYPERBAND HYPERPARAMETERS + RANDOM UNDERSAMPLING (RUS)")
print("="*70)
...
print("✅ Batch Size: 32")
print("✅ Under-sampling: RandomUnderSampler on TRAIN only")
print("="*70)
...
# ================================
# RandomUnderSampler (TRAIN ONLY)
# ================================
print("\n" + "="*50)
print("RANDOM UNDER-SAMPLING (training set only)")
print("="*50)

counts_before = Counter(y_train_enc)
print("Class counts BEFORE RUS:", {class_names[k]: v for k, v in counts_before.items()})

min_class_count = min(counts_before.values())
print(f"Minority class size = {min_class_count} → RUS will reduce majority classes to this size.")

rus = RandomUnderSampler(random_state=42)  # sampling_strategy='auto' → down to minority count
t_us0 = time.time()
X_train_rus, y_train_rus = rus.fit_resample(X_train, y_train_enc)
t_us1 = time.time()

print(f"RUS time: {t_us1 - t_us0:.2f}s")
counts_after = Counter(y_train_rus)
print("Class counts AFTER  RUS:", {class_names[k]: v for k, v in counts_after.items()})
print(f"Training samples after RUS: {X_train_rus.shape[0]} (down from {X_train.shape[0]})")

num_classes = len(class_names)
y_train_rus_cat = tf.keras.utils.to_categorical(y_train_rus, num_classes)

# Guard: if downsampled set is tiny, skip validation split to avoid errors
val_split = 0.15 if X_train_rus.shape[0] >= 200 else 0.0
if val_split == 0.0:
    print("⚠️ Very small training set after RUS → using validation_split=0.0")
...
# ================================
print("\n" + "="*50)
print("BUILDING AND TRAINING MODEL (RUS)")
print("="*50)

model = create_optimal_model(X_train_rus.shape[1], len(class_names))
model.summary()

print("\n--- Training Model with Optimal Hyperparameters + RUS ---")
start_train = time.time()
history = model.fit(
    X_train_rus, y_train_rus_cat,
    epochs=20,
    batch_size=32,
...
per_class_df.to_csv("per_class_metrics_rus.csv", index=False)
with open("per_class_metrics_rus.json", "w") as f:
    json.dump(per_class_df.to_dict(orient="records"), f, indent=2)

print("\nSaved per-class metrics to:")
print("  • per_class_metrics_rus.csv")
print("  • per_class_metrics_rus.json")
...
# ================================
model.save('kdd_optimal_model_rus.keras')

results_summary = {
    "timestamp": datetime.now().isoformat(),
    "model_type": "Optimal Hyperband Model + RUS",
    "hyperparameters": {
        **best_hyperparams,
        "rus_strategy": "auto (to minority)"
    },
...
with open("optimal_model_results_rus.json", 'w') as f:
    json.dump(results_summary, f, indent=4)

print("\n" + "="*60)
print("✅ MODEL TRAINING COMPLETE (RUS)!")
print("="*60)
print(f"🎯 Final Accuracy: {acc*100:.2f}%")
print("📁 Model saved as: kdd_optimal_model_rus.keras")
print("📁 Results saved as: optimal_model_results_rus.json")
print("📁 Per-class CSV:   per_class_metrics_rus.csv")
print("📁 Per-class JSON:  per_class_metrics_rus.json")
