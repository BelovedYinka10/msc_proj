...
# ---------- Random Under Sampling (RUS) inserted here; structure unchanged ----------
print("\n" + "=" * 60)
print("APPLYING RANDOM UNDERSAMPLING (RUS) TO TRAINING SET")
print("=" * 60)
try:
    from imblearn.under_sampling import RandomUnderSampler
    from collections import Counter

    # Fraction of the majority class to keep (0 < RUS_FRACTION <= 1.0)
    ...
    rus_target_for_majority = max(max_minority, int(RUS_FRACTION * majority_count))

    # Build sampling_strategy dict for RandomUnderSampler:
    # Reduce majority to rus_target_for_majority, leave minorities unchanged
    sampling_target = {}
    for cls_idx, cnt in orig_counts.items():
        sampling_target[cls_idx] = cnt if cls_idx != majority_label else rus_target_for_majority

    print("\nRUS sampling targets (per-class):")
    for idx, name in enumerate(class_names):
        tgt = sampling_target.get(idx, 0)
        print(f"  {name}: target -> {tgt}")

    # Apply RandomUnderSampler with dict strategy
    ...
    print("\nAfter RUS training class distribution:")
    for idx, name in enumerate(class_names):
    ...
    print("⚠️ imbalanced-learn (imblearn) not available or RandomUnderSampler failed:", e)
    print("Proceeding WITHOUT RUS. The original training set will be used.")
