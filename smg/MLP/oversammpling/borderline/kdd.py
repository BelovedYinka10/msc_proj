...
# ---------- Borderline-SMOTE sampling (inserted here, structure unchanged) ----------
print("\n" + "=" * 60)
print("APPLYING BORDERLINE-SMOTE (borderline-1) TO TRAINING SET")
print("=" * 60)
try:
    ...
    # Create BorderlineSMOTE instance
    # sampling_strategy='not majority' will resample all minority classes up to majority by default
    # You can change sampling_strategy to a float <1.0 or dict to control exact targets.
    bsmote = BorderlineSMOTE(sampling_strategy='not majority', kind='borderline-1', random_state=42, k_neighbors=5)

    # Fit-resample
    X_train_res, y_train_res = bsmote.fit_resample(X_train, y_train_enc)

    res_counts = Counter(y_train_res)
    print("\nAfter Borderline-SMOTE training class distribution:")
    for idx, name in enumerate(class_names):
    ...
    print("⚠️ imbalanced-learn (imblearn) not available or BorderlineSMOTE failed:", e)
    print("Proceeding WITHOUT Borderline-SMOTE. The original training set will be used.")
