...
# NEW: Borderline-SMOTE + diagnostics
...
print("="*70)
print("KDD MODEL WITH BEST HYPERBAND HYPERPARAMETERS + BORDERLINE-SMOTE")
print("="*70)
...
print("✅ Batch Size: 32")
print("✅ Oversampling: Borderline-SMOTE on TRAIN only")
print("="*70)
...
def preprocess_kdd_5class(train_data, test_data, target_col):
    """Preprocess KDD 5-class data: label-encode categoricals, min-max scale, encode labels."""
    print("\nPreprocessing data...")
...
# ================================
# Borderline-SMOTE oversampling (TRAIN ONLY)
# ================================
print("\n" + "="*50)
print("BORDERLINE-SMOTE OVERSAMPLING (training set only)")
print("="*50)
print("Class counts BEFORE BLS:", {class_names[k]: v for k, v in Counter(y_train_enc).items()})

# kind='borderline-1' (more conservative). Try 'borderline-2' if you want more synthetic samples near majority.
bls = BorderlineSMOTE(random_state=42, k_neighbors=5, m_neighbors=10, kind='borderline-1')
t_os0 = time.time()
X_train_os, y_train_os = bls.fit_resample(X_train, y_train_enc)
t_os1 = time.time()

print(f"BLS time: {t_os1 - t_os0:.2f}s")
print("Class counts AFTER  BLS:", {class_names[k]: v for k, v in Counter(y_train_os).items()})

num_classes = len(class_names)
y_train_os_cat = tf.keras.utils.to_categorical(y_train_os, num_classes)
...
# ================================
print("\n" + "="*50)
print("BUILDING AND TRAINING MODEL (BORDERLINE-SMOTE)")
print("="*50)

model = create_optimal_model(X_train_os.shape[1], len(class_names))
model.summary()

print("\n--- Training Model with Optimal Hyperparameters + Borderline-SMOTE ---")
start_train = time.time()
history = model.fit(
    X_train_os, y_train_os_cat,
    epochs=20,
    batch_size=32,
...
per_class_df.to_csv("per_class_metrics_borderline.csv", index=False)
with open("per_class_metrics_borderline.json", "w") as f:
    json.dump(per_class_df.to_dict(orient="records"), f, indent=2)

print("\nSaved per-class metrics to:")
print("  • per_class_metrics_borderline.csv")
print("  • per_class_metrics_borderline.json")
...
# ================================
model.save('kdd_optimal_model_borderline.keras')

results_summary = {
    "timestamp": datetime.now().isoformat(),
    "model_type": "Optimal Hyperband Model + Borderline-SMOTE",
    "hyperparameters": {
        **best_hyperparams,
    ...
    "train_class_counts_before": {str(class_names[k]): int(v) for k, v in Counter(y_train_enc).items()},
    "train_class_counts_after":  {str(class_names[k]): int(v) for k, v in Counter(y_train_os).items()}
}

with open("optimal_model_results_borderline.json", 'w') as f:
    json.dump(results_summary, f, indent=4)

print("\n" + "="*60)
print("✅ MODEL TRAINING COMPLETE (BORDERLINE-SMOTE)!")
print("="*60)
print(f"🎯 Final Accuracy: {acc*100:.2f}%")
print("📁 Model saved as: kdd_optimal_model_borderline.keras")
print("📁 Results saved as: optimal_model_results_borderline.json")
print("📁 Per-class CSV:   per_class_metrics_borderline.csv")
print("📁 Per-class JSON:  per_class_metrics_borderline.json")
