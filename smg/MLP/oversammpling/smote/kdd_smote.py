...
print("="*70)
print("KDD MODEL WITH BEST HYPERBAND HYPERPARAMETERS + SMOTE")
print("="*70)
...
print("✅ Batch Size: 32")
print("✅ Oversampling: SMOTE on TRAIN only")
print("="*70)
...
print("Class counts BEFORE SMOTE:", {class_names[k]: v for k, v in counts_before.items()})

# Auto-tune k_neighbors to avoid errors when the rarest class is very small
min_class_count = min(counts_before.values())
k_neighbors = max(1, min(5, min_class_count - 1))  # need at least k+1 samples in each minority class
print(f"Using SMOTE with k_neighbors={k_neighbors}")

smote = SMOTE(random_state=42, k_neighbors=k_neighbors)  # can set sampling_strategy if you want partial upsampling
t_os0 = time.time()
X_train_os, y_train_os = smote.fit_resample(X_train, y_train_enc)
t_os1 = time.time()

print(f"SMOTE time: {t_os1 - t_os0:.2f}s")
counts_after = Counter(y_train_os)
print("Class counts AFTER  SMOTE:", {class_names[k]: v for k, v in counts_after.items()})
...
# ================================
print("\n" + "="*50)
print("BUILDING AND TRAINING MODEL (SMOTE)")
print("="*50)

model = create_optimal_model(X_train_os.shape[1], len(class_names))
model.summary()

print("\n--- Training Model with Optimal Hyperparameters + SMOTE ---")
...
per_class_df.to_csv("per_class_metrics_smote.csv", index=False)
with open("per_class_metrics_smote.json", "w") as f:
    json.dump(per_class_df.to_dict(orient="records"), f, indent=2)

print("\nSaved per-class metrics to:")
print("  • per_class_metrics_smote.csv")
print("  • per_class_metrics_smote.json")
...
# ================================
model.save('kdd_optimal_model_smote.keras')

results_summary = {
    "timestamp": datetime.now().isoformat(),
    "model_type": "Optimal Hyperband Model + SMOTE",
    "hyperparameters": {
        **best_hyperparams,
...
with open("optimal_model_results_smote.json", 'w') as f:
    json.dump(results_summary, f, indent=4)

print("\n" + "="*60)
print("✅ MODEL TRAINING COMPLETE (SMOTE)!")
print("="*60)
print(f"🎯 Final Accuracy: {acc*100:.2f}%")
print("📁 Model saved as: kdd_optimal_model_smote.keras")
print("📁 Results saved as: optimal_model_results_smote.json")
print("📁 Per-class CSV:   per_class_metrics_smote.csv")
print("📁 Per-class JSON:  per_class_metrics_smote.json")
