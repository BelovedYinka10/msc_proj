...
print("="*70)
print("KDD MODEL WITH BEST HYPERBAND HYPERPARAMETERS + ADASYN")
print("="*70)
...
print("✅ Batch Size: 32")
print("✅ Oversampling: ADASYN on TRAIN only")
print("="*70)
...
# ================================
# ADASYN oversampling (TRAIN ONLY)
# ================================
print("\n" + "="*50)
print("ADASYN OVERSAMPLING (training set only)")
print("="*50)
print("Class counts BEFORE ADASYN:", {class_names[k]: v for k, v in Counter(y_train_enc).items()})

adasyn = ADASYN(random_state=42, n_neighbors=5)  # try n_neighbors=3 if minority is extremely tiny
t_os0 = time.time()
X_train_os, y_train_os = adasyn.fit_resample(X_train, y_train_enc)
t_os1 = time.time()

print(f"ADASYN time: {t_os1 - t_os0:.2f}s")
print("Class counts AFTER  ADASYN:", {class_names[k]: v for k, v in Counter(y_train_os).items()})
...
# ================================
print("\n" + "="*50)
print("BUILDING AND TRAINING MODEL (ADASYN)")
print("="*50)

model = create_optimal_model(X_train_os.shape[1], len(class_names))
model.summary()

print("\n--- Training Model with Optimal Hyperparameters + ADASYN ---")
...
per_class_df.to_csv("per_class_metrics_adasyn.csv", index=False)
with open("per_class_metrics_adasyn.json", "w") as f:
    json.dump(per_class_df.to_dict(orient="records"), f, indent=2)

print("\nSaved per-class metrics to:")
print("  • per_class_metrics_adasyn.csv")
print("  • per_class_metrics_adasyn.json")
...
# ================================
model.save('kdd_optimal_model_adasyn.keras')

results_summary = {
    "timestamp": datetime.now().isoformat(),
    "model_type": "Optimal Hyperband Model + ADASYN",
    "hyperparameters": {
        **best_hyperparams,
        "adasyn_n_neighbors": 5,
        "adasyn_strategy": "auto"
    },
...
with open("optimal_model_results_adasyn.json", 'w') as f:
    json.dump(results_summary, f, indent=4)

print("\n" + "="*60)
print("✅ MODEL TRAINING COMPLETE (ADASYN)!")
print("="*60)
print(f"🎯 Final Accuracy: {acc*100:.2f}%")
print("📁 Model saved as: kdd_optimal_model_adasyn.keras")
print("📁 Results saved as: optimal_model_results_adasyn.json")
print("📁 Per-class CSV:   per_class_metrics_adasyn.csv")
print("📁 Per-class JSON:  per_class_metrics_adasyn.json")
