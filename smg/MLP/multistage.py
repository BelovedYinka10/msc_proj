...
if passed:
    ...
    if passed_bin:
        ...
        print("\n=== Stage 3: Multi-class on Attacks ===")
        model_attack = build_dnn(X_attack_train.shape[1], y_attack_train.shape[1])
        ...
        if passed_att:
            ...
            while len(np.unique(y_attack_train_idx)) > 2:
                ...
                model_bin_rec = build_dnn(X_attack_train.shape[1], 1, binary=True)
            ...
            if len(np.unique(y_attack_train_idx)) == 2:
                ...
                model_final = build_dnn(X_attack_train.shape[1], 1, binary=True)
