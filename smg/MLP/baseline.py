...
# Model configuration (use hyperparameters)
...
def create_mlp(input_dim, output_dim):
    ...
    optimizer = tf.keras.optimizers.RMSprop(learning_rate=0.0011140493525027336)  # Best lr
...
plt.title('Normal Training and Validation Accuracy')
plt.xlabel('Epoch')
plt.ylabel('Accuracy')
plt.legend()
plt.tight_layout()
...
plt.title('Normal Training and Validation Loss')
plt.xlabel('Epoch')
plt.ylabel('Loss')
plt.legend()
plt.tight_layout()
...
# === Confusion Matrix (counts, blue colormap) ===
cm = confusion_matrix(y_true, y_pred)

plt.figure(figsize=(6, 5))
plt.imshow(cm, interpolation='nearest', cmap="Blues")  # <-- blue colormap
plt.title('MLP Confusion Matrix (Counts)')
...
# === Confusion Matrix (row-normalized, blue colormap) ===
cm_norm = cm.astype('float') / cm.sum(axis=1, keepdims=True)

plt.figure(figsize=(6, 5))
plt.imshow(cm_norm, interpolation='nearest', cmap="Blues")  # <-- blue colormap
plt.title('MLP Confusion Matrix (Normalized by True Class)')
plt.xlabel('Predicted Label')
plt.ylabel('True Label')
plt.colorbar()
plt.xticks(tick_marks, class_labels, rotation=45, ha='right')
