...
# ADASYN expects 1D labels, convert y_train from one-hot to integer labels
...
y_train_resampled = pd.get_dummies(y_train_resampled_labels, columns=y_encoded.columns.tolist())
# Ensure the columns of the resampled one-hot encoded labels match the original
# This is crucial if some classes might not be present in a small y_train subset initially
missing_cols = set(y_encoded.columns) - set(y_train_resampled.columns)
for c in missing_cols:
    y_train_resampled[c] = 0
y_train_resampled = y_train_resampled[y_encoded.columns]
...
# Model configuration
input_dim = X_train_resampled.shape[1] # Use resampled input dim
output_dim = y_train_resampled.shape[1] # Use resampled output dim (should be same as original)
...
# Updated RNN hyperparameters based on the user's request.
# Note: 'filters1' and 'filters2' in the input are mapped to 'lstm_units1' and 'lstm_units2' for the RNN configuration.
rnn_params = {
    'batch_size': 32,
    'lstm_units1': 64,  # Mapped from filters1: 64
    'dropout1': 0.1,
    'lstm_units2': 96,  # Mapped from filters2: 96
    'dropout2': 0.2,
    'dense_units': 128,
    'optimizer': 'adam',
...
# Use batch_size from rnn_params and resampled data for training
history = model.fit(X_train_resampled, y_train_resampled, epochs=10, batch_size=rnn_params['batch_size'], validation_split=0.1, verbose=1)
end_train = time.time()
