# kdd_cnn_from_tuner_values_adasyn.py
# NSL-KDD (5-class) — CNN with tuner-configured Dense head + ADASYN on the training set
...
print("=" * 70)
print("NSL-KDD (5-class) — CNN with tuner Dense head + ADASYN")
print("=" * 70)
for k, v in HP.items():
    print(f"{k}: {v}")
...
# Default: oversample all minority classes up to the majority size
adasyn = ADASYN(random_state=42, n_neighbors=3, sampling_strategy="auto")

# ---- CAP EXAMPLE (optional) ----
# If you want to prevent very large expansion, replace the line above with a dict:
# max_target = 6000  # cap each class to at most this many samples
# target = {}
# for cls, n in cnt_before.items():
#     maj = max(cnt_before.values())
#     desired = min(maj, max_target) if n < maj else n
#     target[cls] = desired
# adasyn = ADASYN(random_state=42, n_neighbors=3, sampling_strategy=target)
# --------------------------------
...
print(f"ADASYN time: {t_oversample_end - t_oversample_start:.2f}s")
...
print(f"\nTrain (after ADASYN): X={X_train_res.shape}, y={y_train_cat.shape}")
...
def build_cnn_with_dense_head(hp: dict, input_dim: int, num_classes: int) -> keras.Model:
    m = keras.Sequential()
    # 1D Conv feature extractor (kept small & fixed)
...
print("\n--- Training CNN (tuner dense head) on ADASYN-resampled train ---")
t0 = time.time()
history = model.fit(
...
plt.title('CNN (ADASYN) — Confusion Matrix (NSL-KDD 5-class)')
plt.tight_layout();
plt.show()
