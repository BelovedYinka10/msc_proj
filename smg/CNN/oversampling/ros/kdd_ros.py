# kdd_cnn_from_tuner_values_ros.py
# NSL-KDD (5-class) — CNN with tuner-configured Dense head + RandomOverSampler (ROS) on the training set
...
print("="*70)
print("NSL-KDD (5-class) — CNN with tuner Dense head + RandomOverSampler (ROS)")
print("="*70)
for k, v in HP.items():
    print(f"{k}: {v}")
...
# -----------------------
# RandomOverSampler (on TRAIN only)
# -----------------------
print("\nClass counts BEFORE ROS:")
cnt_before = Counter(y_train_enc)
for k in sorted(cnt_before):
    print(f"  {class_names[k]:<8} -> {cnt_before[k]}")

ros = RandomOverSampler(
    sampling_strategy="auto",  # oversample all minority classes up to the majority count
    random_state=42
)
...
print(f"\nTrain (after ROS): X={X_train_res.shape}, y={y_train_cat.shape}")
...
print("\n--- Training CNN (tuner dense head) on ROS-resampled train ---")
t0 = time.time()
history = model.fit(
...
plt.title('CNN (ROS) — Accuracy'); plt.xlabel('Epoch'); plt.ylabel('Acc')
plt.legend(); plt.tight_layout(); plt.show()

plt.figure(figsize=(6, 4))
plt.plot(history.history['loss'], label='Train Loss')
plt.plot(history.history['val_loss'], label='Val Loss')
plt.title('CNN (ROS) — Loss'); plt.xlabel('Epoch'); plt.ylabel('Loss')
plt.legend(); plt.tight_layout(); plt.show()
...
plt.title('CNN (ROS) — Confusion Matrix (NSL-KDD 5-class)')
