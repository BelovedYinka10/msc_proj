# kdd_cnn_from_tuner_values_borderlinesmote.py
# NSL-KDD (5-class) — CNN with tuner-configured Dense head + Borderline-SMOTE on the training set
...
print("="*70)
print("NSL-KDD (5-class) — CNN with tuner Dense head + Borderline-SMOTE")
print("="*70)
for k, v in HP.items():
    print(f"{k}: {v}")
...
# -----------------------
print("\nClass counts BEFORE Borderline-SMOTE:")
cnt_before = Counter(y_train_enc)
for k in sorted(cnt_before):
    print(f"  {class_names[k]:<8} -> {cnt_before[k]}")

# Borderline-1 variant focuses on borderline minority samples
bls = BorderlineSMOTE(
    sampling_strategy="auto",   # oversample all minorities up to majority
    random_state=42,
    k_neighbors=5,              # neighbors for minority samples
    m_neighbors=10,             # neighbors to find danger set
    kind="borderline-1"         # or "borderline-2"
)

t_oversample_start = time.time()
X_train_res, y_train_res = bls.fit_resample(X_train, y_train_enc)
t_oversample_end = time.time()

print("\nClass counts AFTER Borderline-SMOTE:")
cnt_after = Counter(y_train_res)
for k in sorted(cnt_after):
    print(f"  {class_names[k]:<8} -> {cnt_after[k]}")
print(f"Borderline-SMOTE time: {t_oversample_end - t_oversample_start:.2f}s")
...
print(f"\nTrain (after Borderline-SMOTE): X={X_train_res.shape}, y={y_train_cat.shape}")
...
print("\n--- Training CNN (tuner dense head) on Borderline-SMOTE-resampled train ---")
t0 = time.time()
history = model.fit(
...
plt.title('CNN (Borderline-SMOTE) — Accuracy'); plt.xlabel('Epoch'); plt.ylabel('Acc')
plt.legend(); plt.tight_layout(); plt.show()

plt.figure(figsize=(6, 4))
plt.plot(history.history['loss'], label='Train Loss')
plt.plot(history.history['val_loss'], label='Val Loss')
plt.title('CNN (Borderline-SMOTE) — Loss'); plt.xlabel('Epoch'); plt.ylabel('Loss')
plt.legend(); plt.tight_layout(); plt.show()
...
print("\n=== Overall Metrics (Borderline-SMOTE) ===")
...
plt.title('CNN (Borderline-SMOTE) — Confusion Matrix (NSL-KDD 5-class)')
...
print("\n=== Per-Class Metrics (Borderline-SMOTE) ===")
