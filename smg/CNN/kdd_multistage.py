...
LABEL_COL = "class5"   # NSL-KDD 5-class label
F_MIN     = 0.60
...
# =======================
# CNN model (your hyperparams)
# =======================
def build_cnn(input_dim: int, output_dim: int) -> keras.Model:
    ...
    opt = keras.optimizers.Adam(learning_rate=0.0032164230487929987)
...
# =======================
# Multi-Stage Pipeline (same structure; CNN replaces GB)
# =======================
...
print("\n=== Stage 1: CNN Multi-class (all classes) ===")
model_multi = build_cnn(input_dim, len(class_names))
t0 = time.time()
...
t1 = time.time()
y_pred_multi = np.argmax(model_multi.predict(X_test, verbose=0), axis=1)
t2 = time.time()
...
if passed1:
    ...
    print("\n=== Stage 2: CNN Binary (Normal vs Attack) ===")
    model_bin = build_cnn(input_dim, 2)
    t0 = time.time()
    model_bin.fit(
        X_train, y_train_bin,
        epochs=10, batch_size=32,
    ...
    t1 = time.time()
    y_pred_bin = np.argmax(model_bin.predict(X_test, verbose=0), axis=1)
    t2 = time.time()
    stage_train = t1 - t0; stage_test = t2 - t1
    total_train_time += stage_train; total_test_time += stage_test
    ...
    if not passed2:
        ...
        # remap attack labels to 0..K-1 for Keras
        ...
        print("\n=== Stage 3: CNN Multi-class (attacks only) ===")
        model_attack = build_cnn(input_dim, len(attack_class_names))
        t0 = time.time()
        model_attack.fit(
            X_attack_train, y_attack_train_m,
            epochs=10, batch_size=32,
            validation_split=0.10, verbose=0
        )
        t1 = time.time()
        y_pred_attack = np.argmax(model_attack.predict(X_attack_test, verbose=0), axis=1)
        t2 = time.time()
        stage_train = t1 - t0; stage_test = t2 - t1
        total_train_time += stage_train; total_test_time += stage_test
        ...
        if passed3:
            ...
            while len(np.unique(y_curr_train)) > 2 and len(y_curr_test) > 0:
                ...
                # Binary labels: 0 if picked, 1 if others (to match label order in prints)
                y_bin_train = (y_curr_train != pick_local).astype(int)
                y_bin_test  = (y_curr_test  != pick_local).astype(int)

                print("\n=== Stage 4: CNN Binary (picked vs others) ===")
                model_bin_rec = build_cnn(input_dim, 2)
                t0 = time.time()
                model_bin_rec.fit(
                    X_curr_train, y_bin_train,
                    epochs=10, batch_size=32,
                    validation_split=0.10, verbose=0
                )
                t1 = time.time()
                y_pred_b = np.argmax(model_bin_rec.predict(X_curr_test, verbose=0), axis=1)
                t2 = time.time()
                stage_train = t1 - t0; stage_test = t2 - t1
                total_train_time += stage_train; total_test_time += stage_test
            ...
            if len(np.unique(y_curr_train)) == 2 and len(y_curr_test) > 0:
                ...
                y_final_train = (y_curr_train != local_a).astype(int)
                y_final_test  = (y_curr_test  != local_a).astype(int)

                print("\n=== Stage 4 Final: CNN Binary (last two attacks) ===")
                model_final = build_cnn(input_dim, 2)
                t0 = time.time()
                model_final.fit(
                    X_curr_train, y_final_train,
                    epochs=10, batch_size=32,
                    validation_split=0.10, verbose=0
                )
                t1 = time.time()
                y_pred_final = np.argmax(model_final.predict(X_curr_test, verbose=0), axis=1)
                t2 = time.time()
                stage_train = t1 - t0; stage_test = t2 - t1
                total_train_time += stage_train; total_test_time += stage_test
